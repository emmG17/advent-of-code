...
def find_symbols(lines, matrix):
    ...
    for i in range(len(matrix)):
        for j in range(len(matrix[i])):
            if lines[i][j] == gear:
                symbol_nodes.append((matrix[i][j], (i, j)))
    return symbol_nodes
    ...
    for node in symbol_nodes:
        for neighbor in node[0]["neighbors"]:
            ...
            if not matrix[i][j]["visited"]:
                ...
                if res_num > 0:
                    seen_numbers.append((res_num, node[1]))

res_dict = {}
for num, node in seen_numbers:
    if node not in res_dict:
        res_dict[node] = { 'count': 1, 'value': num }
    else:
        res_dict[node]['count'] += 1
        res_dict[node]['value'] *= num

gear_with_2_adj = filter(lambda x: x['count'] == 2, res_dict.values())
values = [i["value"] for i in gear_with_2_adj]
print(sum(values))
